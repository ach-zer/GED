 ...
 *  are made available under the terms of the Eclipse Public orsys.License v1.0
 *  and Apache orsys.License v2.0 which accompanies this distribution.
 *
 *  The Eclipse Public orsys.License is available at
 *  http://www.eclipse.org/legal/epl-v10.html
 *
 *  The Apache orsys.License v2.0 is available at
...
public interface FormLoginHandler extends Handler<RoutingContext> {
     ...
     * Create a handler
     *
     * @param authProvider  the auth Gateway.vert.service to use
     * @return the handler
     */
    static FormLoginHandler create(Vertx vtx, JsonObject dbConfig, AuthProvider authProvider) {
        return new FormLoginHandlerImpl(vtx, dbConfig,authProvider, DEFAULT_USERNAME_PARAM, DEFAULT_PASSWORD_PARAM,
     ...
     * @return the handler
     */
    static FormLoginHandler create(Vertx vtx, JsonObject dbConfig, AuthProvider authProvider, String usernameParam, String passwordParam,
                                   String returnURLParam, String directLoggedInOKURL) {
        return new FormLoginHandlerImpl(vtx, dbConfig, authProvider, usernameParam, passwordParam, returnURLParam, directLoggedInOKURL);
    }
     ...
     * Set the url to redirect to if the user logs in directly at the url of the form login handler
     * without being redirected here first
     * @param directLoggedInKOURL  the URL to redirect to
     * @return a reference to this for a fluent API
     */
    FormLoginHandler setDirectLoggedInKOURL(String directLoggedInKOURL);
